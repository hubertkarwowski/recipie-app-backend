...
def get_recipe_links(soup):
    for script in soup.select("script[type='application/ld+json']"):
        try:
            ...
            logger.warning("Failed to parse JSON-LD: {e}", e=e)
...
def get_all_links():
    ...
    logger.info(f"Liczba stron: {total_pages}")
    ...
    for page in range(1, total_pages + 1):
        ...
        logger.info(f"Strona {page}/{total_pages} — znaleziono {len(links)} linków")

    logger.info(f"Łącznie: {len(all_links)} przepisów")
    return all_links
...
def parse_recipe(url):
    ...
    DIET_MAP = {
        "VegetarianDiet": Diet.vegetarian,
        "VeganDiet": Diet.vegan,
        "GlutenFreeDiet": Diet.gluten_free,
        "RestrictedDiet": Diet.low_sugar,
        "LowCalorieDiet": Diet.low_calorie,
        "LowFatDiet": Diet.low_fat,
    }
    diets: set[Diet] = set()
...
def save_recipe(data: RecipeData, url: str):
    with Session(engine) as db:
        ...
        if existing:
            logger.warning(f"Przepis już istnieje w bazie: {data.title}")
            return
        ...
        recipe = Recipe(
            ...
            image_url=data.img,
            source_url=url,
            labels=list(diet.value for diet in data.diets),
        ...
        for ing in data.ingredients:
            db.add(Ingredient(recipe_id=recipe.id, name=ing.name, qty=ing.qty))

        db.commit()
        logger.info(f"Zapisano przepis: {data.title}")
...
def scrape_recipes():
    ...
    for i, url in enumerate(links[:5], 1):
        try:
            ...
            logger.error("Error URL: {url} - {e}", url=url, e=e)
